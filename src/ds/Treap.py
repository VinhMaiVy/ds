...
class Treap:
    ...
    def merge(self, left, right):
        ...
        #print('left:', left.data, left.priority, left.size, 'right:', right.data, right.priority, right.size)
    ...
    def split(self, index):
        ...
        if index == 1:
            #print('index is 1, cut off', t.right.data if t.right else 'None', 'and return', t.data)
            temp = self.right
            self.right = None
            self.size -= temp.size
            return self, temp
        ...
        self.size -= leftover.size #leftover exists because we already checked if they wanted everyhing at the top
